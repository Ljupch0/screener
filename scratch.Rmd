...
```{r}
testJSON <- getIncomeJSON(c("GOOG", "MSFT", "FB"), report="yearly")
```
...
getIncomeJSON <- function (ticker, report="yearly") {
  ...
  df <- bind_rows(l) %>% 
    ...
    ifelse("researchDevelopment.raw" %in% colnames(df), df <- df %>% mutate(rd = parse_number(researchDevelopment.raw)), df <- df %>%  mutate(rd=NA))
