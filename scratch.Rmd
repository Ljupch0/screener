...
```{r}
getIncomeMultiple <- function (ticker) {
  for (i in ticker){
    getIncome(i)
  }
  ticker_statements <- vector()
  for (i in ticker){
    ticker_statements[i]=paste0(i, "_income")
  }
  income_sta
  tements bind_rows(ticker_statements)
}
```
...
for (i in ticker){
  ...
  for (i in ticker){
    ticker_statements[i]=paste0(i, "_income")
    ticker_statements <- nchar(unlist(lapply(ticker_statements,function(s) eval(parse(text=s)))))
  }
  income_statements <- bind_rows(ticker_statements)
...
url_quarterly <- paste0("https://query2.finance.yahoo.com/v10/finance/quoteSummary/",ticker,"?modules=incomeStatementHistoryQuarterly")
