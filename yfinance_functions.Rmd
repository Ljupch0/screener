...
getBalanceSheet <- function (ticker, format="tidy") {
  ...
  if (format=="tidy") {
    assign(paste0(tolower(ticker),"_bs"), tidy(clean(scrape(ticker))), envir = .GlobalEnv)
  } else if (format=="clean") {
    assign(paste0(tolower(ticker),"_bs_clean"), clean(scrape(ticker)), envir = .GlobalEnv)
  } else if (format=="raw") {
    assign(paste0(tolower(ticker), "_bs_raw"), scrape(ticker), envir = .GlobalEnv)
  } else {
    stop('Format can only be "tidy", "clean", or "raw".')
  }
...
```{r}
getFinancials <- function (ticker, format="tidy") {
  getIncome(ticker, format)
  getBalanceSheet(ticker, format)
  getCashFlow(ticker, format)
}
```
