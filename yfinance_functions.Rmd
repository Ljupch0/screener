...
getIncome <- function (ticker, format = "tidy", assign = TRUE) {
  ...
  if (format=="tidy") {
    ...
    stop('Format can only be "tidy", "clean", or "raw", and assign can be TRUE or FALSE')
  }
...
scrape <- function (ticker) {
  ...
  ifelse(is.null(df),
         stop("After 5 attempts, no data was downloaded from yfinance. Either your ticker is wrong, or you have been downloading a lot of data and yfinance is blocking you."),
         return(df))
}
...
```{r}
getBalanceSheet("AAPL","tidy", FALSE)
```



```{r}
getBalanceSheet("AAPL", "clean", assign=FALSE)
```
...
```{r}
getFinancials("KO", assign=FALSE) %>% 
  dplyr::select(ends_with(".x"),ends_with(".y"))


#clean up duplicates, rename pseudo-duplicates
```



```{r}
getIncome(ticker, format="tidy", assign=FALSE)
```
